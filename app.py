...
class Drawing:
    ...
    def left_right(self, isLeft:bool):
        heading = self.turt.heading() + 20 if isLeft else self.turt.heading() - 20
        self.turt.setheading(heading)
        self.turt.forward(20)
    def forward_backward(self, isFor:bool):
        self.turt.forward(10) if isFor else self.turt.backward(10)
    def clearScreen(self):
        self.turt.clear()  # Clears turtle drawings
        self.turt.penup()
        self.turt.home()  # Move turtle to its home position (starting position)
        self.turt.pendown()
    def run(self):
        self.screen.listen()
